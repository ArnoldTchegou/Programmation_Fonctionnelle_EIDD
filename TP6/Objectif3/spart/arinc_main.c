...
void t1(){
  ...
  for(;;){
    ...
    PERIODIC_WAIT(&rc) ;
    console_perror(rc,"spart","t1:PERIODIC_WAIT");
  }
}
...
void main_process() {
  ...
  console_perror(rc,"spart","main_process:CREATE_PROCESS");
  // Start process t1. This places it into waiting mode.
  // Actual start will take place at the temporal reference
  // point.
  START(t1_pid,&rc) ;
  console_perror(rc,"spart","main_process:START");
  // Enter NORMAL mode
  SET_PARTITION_MODE(NORMAL,&rc) ;
  console_perror(rc,"spart","main_process:SET_PARTITION_MODE");
